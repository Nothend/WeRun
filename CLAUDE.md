...
Key env vars to set in ECS `.env`: `APPID`, `APPSECRET`, `DASHSCOPE_API_KEY`, `JWT_SECRET`, `PUBLIC_BASE_URL=https://your.domain.com`, `WERUN_IMAGE`. Optional tuning: `MIN_DURATION_MINUTES` (default 30), `WEEKLY_TARGET` (default 3), `SCREENSHOT_MAX_LAG_DAYS` (default 1 — screenshot exercise date may be at most this many days before today), `SHOW_SUPPORT` (default false — shows the about-page cost/donation section; keep false when submitting for platform review), `AUTO_APPROVE_MEMBERS` (default false — when true, a user's join application is auto-approved to `active` immediately on submit; admins are still notified, they just don't need to review), `SPONSOR_OPENIDS` (comma-separated allowlist — these members get a "prestigious" gold treatment on leaderboards/feed/profile via the `isSponsor` flag added to user objects in `stats.js`/`login.js`/`profile.js`; `config.isSponsor()` is the single source of truth, no DB column. Each entry may carry a per-member badge as `openid:badge` — e.g. `oAAA,oBBB:🍒,oCCC:🎀` — so different sponsors can show different emoji; entries with no `:badge` fall back to `SPONSOR_BADGE`), `SPONSOR_BADGE` (default `💎` — the default badge for sponsors who didn't specify their own; `config.sponsorBadgeFor(openid)` resolves each sponsor's badge and it is baked into the user object as `sponsorBadge` next to `isSponsor`, so the mini program renders each user's own badge — no global `/api/config` field. Empty string hides the badge while keeping the avatar ring + name gradient), `IMAGE_SIMILARITY_BLOCK_THRESHOLD` / `IMAGE_SIMILARITY_LOG_THRESHOLD`, `APPLY_TEMPLATE_ID` / `WEEKLY_TEMPLATE_ID` (subscribe messages), `QWEN_MODEL` / `QWEN_TEXT_MODEL`.
